...
public class SlideoutMenuFragment extends Fragment implements OnClickListener {
	...
	public static final String FRAGMENT_TAG_LIGHT = "light";
...
//			FeatureFlag.ALARM, FeatureFlag.FOLDER };
	private static int sFeature[] = new int[] {};
	...
	/**
	 * @param tag
	 *            it point to normal mode,like music,radio,line in..
	 * @param specialCataLogSelected
	 *            while true,mean special catalog was been selected.
	 * @param selectedMode
	 *            if parameter SpecialCataLog was selected,it become valid and
	...
	public void setMenuSelected(String tag, boolean specialCataLogSelected, int selectedMode) {
		...
		if (!specialCataLogSelected) {
			...
			} else if (tag.equalsIgnoreCase(FRAGMENT_TAG_LIGHT)) {
				mLightBtnLayout.setSelected(true);
			} 
		} else {
			mFolderEntryList = mBluzManager.getMusicFolderList();
			for (int i = 0; i < mFolderEntryList.size(); i++) {
	...
	private void setMenuUnselected() {
		...
		mLightBtnLayout.setSelected(false);
	...
	public void onClick(View v) {
		...
		if (v.getId() == R.id.connectionBtnLayout) {
			...
			switch (v.getId()) {
			case R.id.a2dpBtnLayout:
				mode = FuncMode.A2DP;
				break;
			case R.id.musicplayBtnLayout:
				mode = FuncMode.CARD;
				break;
			...
			case R.id.recUhostPlayBackBtnLayout:
				...
				break;
			case R.id.lightBtnLayout:
				//灯使用的闹铃的模式
				mode = BluetoothLightCommond.FuncMode.LIGHT;
				break;
			default:
				// Special Catalog
				for (int i = 0; i < mFolderEntryList.size(); i++) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		//下面会屏蔽掉，闹铃、音乐、收音机、A2DP
		View view = inflater.inflate(R.layout.main_left_fragment, container, false);
		mA2DPBtnLayout = view.findViewById(R.id.a2dpBtnLayout);
		mA2DPBtnLayout.setOnClickListener(this);
		mA2DPBtnLayout.setVisibility(View.GONE);
		...
		mAlarmClockBtnLayout.setVisibility(View.GONE);
		...
		mLightBtnLayout = view.findViewById(R.id.lightBtnLayout);
		mLightBtnLayout.setOnClickListener(this);
		// Expand
